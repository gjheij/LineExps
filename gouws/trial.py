...
class TwoSidedTrial(Trial):
    ...
    def draw(self):
        ...
        if self.phase == 1:
            ...
            phase = np.fmod(self.session.timer.getTime() - self.presentation_time, 1.0/self.session.hemistim.frequency) * self.session.hemistim.frequency * self.parameters['direction']
            self.presentation_time = self.session.timer.getTime()

            if self.parameters['direction_changes'] == 1:
                if (self.presentation_time > -self.session.settings['design'].get('stim_duration')/2):
                    print('switch direction')
                    phase = -phase

            self.session.hemistim.draw(phase=phase)
